...
func (service *UserService) GetCookieAndAuth(number, password string) (string, string) {
	ji, rp := service.NewCCNUClient(number, password)
	pt, _ := service.Index(ji, rp)
...
// 获取JESSIONID和routeportal
func (service *UserService) NewCCNUClient(studentID string, password string) (string, string) {
...
// 获取PORTAL_TOKEN
func (service *UserService) Index(ji, rp string) (string, error) {
	// 创建包含Cookie的请求
